...
class StarField:
    ...
    def download_star_field(self, apply_filtering=False, max_stars=10000):
        ...
        print(f"Координаты запроса: RA={ra_str}, DEC={sign}{de_str}")
        print(f"Предел звёздной величины: {self.config.maglim}")

        # Download data
        try:
            print("Загрузка таблицы координат/величин звёздного поля...")
            response = requests.get(base_url, params=params)

            if response.status_code != 200:
                print(f"Ошибка загрузки данных звёзд: HTTP {response.status_code}")
                return False

            # Check for server error in response
            if 'stat="ERROR"' in response.text:
                print("Сервер вернул ошибку в ответе")
            ...
            print(f"Данные звёзд сохранены в {star_data_file}")

            # Check if the file contains actual data
            with open(star_data_file, 'r') as f:
                content = f.read()
                if '|' not in content:
                    print("Предупреждение: в загруженном файле отсутствует ожидаемый разделитель столбцов '|'")
    ...
    def process_star_field(self, apply_filtering=False):
        ...
        try:
            ...
            with open(output_file, 'w') as star_file:
                print(f"Обработка данных звёзд из {star_data_file}")
                ...
                print(f"Обработано {processed_count} звёзд, добавлено {self.star_count} в пределах лимита звёздной величины {self.config.maglim}")
