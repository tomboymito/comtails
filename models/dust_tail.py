...
class DustTail:
            ...
            for itime in range(config.ntimes - 1):
                ...
                if config.iejec_mode == 3 and config.isun == 1:
                    ...
                    if nshadow == config.nevent:
                        print(f"Вся заданная активная область находится в тени при: "
                        ...
                        print(f"  Применяется red_factor={red_factor:.4f} к темпу пылеобразования")
                        dmdt *= red_factor
                        xmass *= red_factor
    ...
    def _interp5(xdtime, config):
        ...
        if xdtime < config.dtime[0] or xdtime > config.dtime[config.ninputs - 1]:
            print(f"Время до перигелия={xdtime} вне диапазона, используется ближайшее значение")
