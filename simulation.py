"""Главный контроллер астрофизической симуляции COMTAILS (динамика частиц + фотометрический выход)."""
import time
import numpy as np
        ...
        for ii in range(self.config.nx):
            for jj in range(self.config.ny):
                if self.config.flux[ii, jj] > 0.0:
                    ...
                    flux_mag[ii, jj] = FLOAT_TYPE(100.0)   # Комментарий (RU): астрофизическая логика и назначение описаны в коде.
