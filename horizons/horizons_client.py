...
class HorizonsClient:
    ...
    def _send_request(self, query_data):
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Ошибка получения данных из JPL: {response.status_code}")
                return ""

            return response.text

        except Exception as e:
            print(f"Ошибка запроса к JPL: {e}")
            return ""
    ...
    def _parse_earth_position(self, response):
        ...
        if not earth_position_found:
            print("ПРЕДУПРЕЖДЕНИЕ: в ответе JPL не удалось найти корректные координаты Земли")
