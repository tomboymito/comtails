...
class SimulationConfig:
    ...
    def setup_image_grid(self, comet):
        ...
        if self.igrapho == 1:
            ...
            print(f"Будет построено примерно {expected_count} частиц ({(1.0 - self.pcp) * 100:.1f}% от общего числа)")
    ...
    def setup_coordinate_system(self, xarr, yarr, zarr, trueanarr, heliorbit, comet):
        ...
        print(f"  Гелиоцентр. координаты Земли на дату наблюдения:   {self.xe:8.7f}  {self.ye:8.7f}  {self.ze:8.7f}")
        print(f"  ЦЕЛЬ RA (ч,м,с):    {self.rah:2d} {int(self.ram):2d} {self.ras:5.2f}")
        print(f"  ЦЕЛЬ DEC (°,' ,\"):   {self.ded:2d} {int(self.dem):2d} {self.des:5.2f}")
        print(
            f"  Фазовый угол = {self.phas_ang:7.3f} Позиционный угол = {self.psang:7.3f} Угол плоскости= {self.plang:7.3f} (град)")
        print(f"  Элементы орбиты кометы: e= {comet.ec:9.6f} q= {comet.qr:9.6f} а.е.    Tп= {comet.per_jd:12.5f}")
        print(f"  Элементы орбиты кометы: Ω= {comet.om:7.3f} ω= {comet.wper:7.3f} i= {comet.inc:7.3f} (град)")
        print(f"  Гелиоцентрическое расстояние цели= {self.rcobs:10.3f} а.е.  Расстояние до наблюдателя= {self.delta:10.3f} а.е.")
